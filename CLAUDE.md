...
| `ROADMAP.md` | План унификации из волн; Волны 1–3 завершены (общее ядро реально `@require`'ится всеми 7 скриптами, UX/конфиг унифицированы), Волна 4.1–4.5 завершены (ESLint, Prettier, GitHub Actions CI, инсталлятор в README, метаблок-валидатор `scripts/validate-meta.js`). Волна 5 частично (unit-тесты чистых парсеров через vitest; jsdom-тир на реальных HTML-фикстурах из `fixtures/` — пока только для `agis-loan-info-navbar.user.js`). Не начаты: `release.yml`, проверка бампа `@version` на PR, TypeScript. Таблица версий скриптов **может устаревать быстрее, чем этот файл** — сверяйся с реальным `@version` в самом скрипте и `git tag -l` для тегов ядра. |
