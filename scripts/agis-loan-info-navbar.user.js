...
(function () {
  ...
  function waitForNavbar() {
    return waitForElement(NAVBAR_SELECTORS.join(', '), { timeout: WAIT_TIMEOUT }).catch((err) => {
      console.warn(
        `[${SCRIPT_NS}] Навбар не найден за ${WAIT_TIMEOUT} мс (${location.href}).`,
        'Пробовались селекторы:',
        NAVBAR_SELECTORS.join(', '),
      );
      throw err;
    });
  }
    ...
    const obs = new MutationObserver((mutations) => {
      for (const m of mutations) {
        for (const node of m.addedNodes) {
          ...
          if (node.matches?.('table, tbody, tr, th, td')) {
            callback(node);
            continue;
          }
  ...
  async function fetchFromBackend(context) {
    ...
    try {
      ...
      throw new Error(`fetchFromBackend: ${err.message}`, { cause: err });
    }
  ...
  function render(context, data, navbar) {
    ...
    [
      ...
      .filter(Boolean)
      .forEach((item) => bar.appendChild(item));
