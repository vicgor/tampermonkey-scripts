...
(function () {
  ...
  async function storageDelete(key) {
    ...
    catch (error) { console.warn('[agis-core] GM_deleteValue ошибка:', key, error); }
  }
  ...
  function ruMonthNumber(rawMonthText) {
    const key = String(rawMonthText || '').trim().toLowerCase();
    if (!key) return null;
    return RU_MONTHS.get(key)
      ?? RU_MONTHS.get(key.replace(/\.$/, '') + '.')
      ?? RU_MONTHS.get(key.replace(/\.$/, ''))
      ?? null;
  }
