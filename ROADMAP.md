...
- [ ] **Метаблок-валидатор** `scripts/validate-meta.js`: проверяет что для каждого `GM_xmlhttpRequest` в коде есть `@connect`, для каждого `GM_setValue` / `GM_getValue` — соответствующий `@grant`, `@version` увеличен если файл изменён. Частично уже покрыто `eslint-plugin-userscripts` (Волна 4.1) — проверить, что реально осталось непокрытым, прежде чем писать отдельный скрипт.
...
- [x] **Unit-тесты парсеров.** `vitest` (`npm test`, см. `README.md` "Тесты"),
  ...
  guard `if (typeof module !== 'undefined' && module.exports) { ...; return; }` в
  начале IIFE (или в конце, если функция зависит от констант ниже по файлу —
  случай `ruMonthNumber`/`RU_MONTHS`); в Tampermonkey `module` не определён, блок
  мёртвый код. `ci.yml` гоняет `npm test` на каждый PR/push.
  Не покрыто (нужен DOM/`jsdom` или рефакторинг, чтобы не зависеть от
  `ruMonthNumber`/DOM-параметров): `resolveGateway`, `normalizeDate`,
  `formatDateDDMMYY`, `applyDateFormatting`, `statusColor`, `parseDoc`, `cellText`,
  `toDateTimeString` (недетерминирован — использует `new Date()`).
...
- [ ] **HTML-фикстуры** страниц AGIS в `fixtures/` для локального прогона скрипта без прода (Storybook-like sandbox).
  Разблокирует `parseDoc`/`getRowValue`/`getRowCell` через `jsdom` — следующий кандидат для unit-тестов.
...
- [ ] **Устранить дублирование `cellText`/`normalizeText`** между `agis-duplicate-income.user.js`/
  ...
  (найдено при подготовке unit-тестов — разные реализации `normalizeText`, navbar-версия
  не убирает `\u00a0`) — кандидат на вынос в `lib/agis-core.js` по аналогии с `ruMonthNumber`.
...
| Волна 4.3–4.5 (CI + релиз) | средний | 1 день | частично (`ci.yml` + инсталлятор есть, `release.yml`/валидатор — отложены) |
...
1. Волна 5: unit-тесты на чистые парсеры (`parseDoc`, `normalizeDate`, `extractTotal`) через `vitest` — именно в этом классе кода находились реальные баги в ходе Волны 2/3 (например, `\b` не работающий на кириллице).
2. `release.yml` и метаблок-валидатор — отложены, брать по мере необходимости.
