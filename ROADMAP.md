...
- [x] **HTML-фикстуры + jsdom (частично).** `fixtures/agis-loan-edit.html` — реальный
  ...
  `extractValue(dc, 'Просрочен на:', /$/)` не останавливается перед `Дата возврата:`
  (стоп-регексп — только конец строки), поэтому `overdueDays` включает в себя ещё и дату
  возврата. Зафиксировано тестом как текущее поведение, не исправлено в этом PR
  (production-логика, не test-инфраструктура).
...
- [ ] **Устранить дублирование `cellText`/`normalizeText`** между `agis-duplicate-income.user.js`/
  `agis-protocol-income-fill.user.js` и `agis-loan-info-navbar.user.js`/`agis-protocol-income-fill.user.js`
  (найдено при подготовке unit-тестов — та же логика продублирована в трёх файлах). Уточнение
  по итогам работы над jsdom-фикстурой: nbsp (\u00a0) в *обеих* реализациях `normalizeText`
  (navbar и protocol-income-fill) убирается одинаково — `\s+` в JS уже включает nbsp,
  явный `.replace(/\u00a0/g, ' ')` в protocol-income-fill избыточен, а не восполняет
  реальный пробел в navbar-версии (более ранняя запись в этом файле про разницу в
  обработке nbsp была неверной, проверено `node -e` на обеих функциях). Дублирование
  всё ещё стоит убрать — просто не из-за этой мнимой разницы в поведении, а из-за
  повторения одной и той же логики в трёх файлах — кандидат на вынос в `lib/agis-core.js`
  по аналогии с `ruMonthNumber`.
...
1. Исправить `overdueDays`, захватывающий `Дата возврата:` в `parseDoc` (найдено jsdom-фикстурой, см. Волна 5 выше) — реальный баг парсинга, не срочный (поле уже используется только для отображения), но теперь есть тест, который зафиксирует правильное поведение после фикса.
2. Устранить дублирование `cellText`/`normalizeText` (см. Волна 5 выше) — по аналогии с `ruMonthNumber`.
3. Собрать HTML-фикстуры для `resolveGateway`/`normalizeDate`/`cellText` (duplicate-income/protocol-income-fill) — продолжение jsdom-тира.
4. `release.yml` и проверка бампа `@version` на PR — отложены, брать по мере необходимости.
