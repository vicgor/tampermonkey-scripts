...
| Волна 4.3–4.5 (CI + релиз) | средний | 1 день | частично (`ci.yml` есть, `release.yml`/инсталлятор/валидатор — нет) |
...
1. `release.yml` — если нужна публикация в GitHub Release при теге, или инсталлятор в README — что важнее для реального использования.
2. Метаблок-валидатор — сначала проверить, что осталось непокрытым `eslint-plugin-userscripts`.
