...
частично (unit-тесты + jsdom-тир завершены, TypeScript не начат).
...
- `agis-deposit-refund` с 2.1.0 работает на всех 11 доменах (канон выполнен), но вручную
  проверен только на `agis.moneymania.ru`. Решение осознанное: поиск полей идёт скорингом
  подписей, а не селекторами конкретного бренда; при несовпадении скрипт не заполняет
  поля, а показывает баннер об ошибке и пишет warn в Console. Отправка формы всегда
  ручная, поэтому максимальный риск на непроверенном бренде — частично заполненная форма,
  которую оператор видит до отправки. Smoke-test на остальных 10 брендах — открытая задача.
...
- [x] **Проверка бампа `@version` на PR.** `scripts/check-version-bump.js`
  (`npm run check-version-bump`, шаг в `ci.yml`) — если `scripts/*.user.js`
  изменился между базовой веткой PR и HEAD, а `@version` в нём — нет, `ERROR`
  (см. README.md "Версионирование": даже патч-правка типа/комментария требует
  бампа). `execFileSync('git', [...])`, не `exec()` со строкой — аргументы не
  проходят через шелл (по итогам security-хука при первой попытке через
  `execSync` со строковой интерполяцией). Требует полной git-истории —
  `checkout` в `ci.yml` теперь `fetch-depth: 0` (был shallow `depth: 1`),
  скрипт сам `git fetch origin <base>` без ограничения глубины (на shallow
  fetch `--depth=1` `merge-base` не всегда находится, если PR разошёлся с
  `main` больше, чем на 1 коммит — проверено вручную на реальной shallow-копии
  репозитория, а не только на локальном full clone). Вне PR-контекста
  (`GITHUB_BASE_REF` не задан — push в `main`, локальный запуск) молча
  пропускает проверку, не блокируя `npm run lint`. Проверено на всех трёх
  сценариях (файл изменён без бампа → `ERROR`; файл изменён с бампом → `OK`;
  не PR-контекст → пропуск) на реальных scratch-ветках.
- [ ] **`release.yml`** — **отложено осознанно**, не просто «не начато». Исходная
  формулировка (`dist/*.user.js` в GitHub Release) не соответствует архитектуре
  репозитория: сборки нет вообще (см. `CLAUDE.md` — скрипты ставятся как есть),
  и Tampermonkey уже автообновляется с `main` через `@updateURL`/`@downloadURL`
  в каждом скрипте, GitHub Release для этого не нужен. Если понадобится —
  скорее всего как changelog-артефакт при новом теге `lib/agis-core.js`
...
- [x] **HTML-фикстуры + jsdom (частично).** `fixtures/agis-loan-edit.html` — реальный
  ...
  `resolveGateway`/`normalizeDate` (duplicate-income) **разблокированы без фикстуры** —
  оказались чистыми строковыми функциями (не DOM), блокировал только TDZ на
  `GATEWAY_MAP`/`ruMonthNumber`, решено той же схемой `let` + guard, что и выше.
  `getHeaderMap` (protocol-income-fill) — **завершено**, `fixtures/agis-task-protocol-list.html`
  (реальный кусок страницы `/admin/supportprocess/domain/supportprocesstask/<id>/task-protocol/list`,
  без анонимизации — на странице нет персональных данных, только id/даты/суммы/статусы),
  тесты в `test/scripts/agis-protocol-income-fill.dom.test.js`. Реальная фикстура снова
  вскрыла настоящую находку (не баг, но не очевидна из кода): на этой странице **нет
  колонки "Номер заказа" вообще** — `getHeaderMap` ищет `'номер заказа'` по всем `th`, такого
  заголовка не существует, `colIndex.orderNumber` всегда `undefined` здесь. Вызывающий код
  (`initListPage`) уже это учитывает (`!== undefined` перед чтением ячейки, иначе `'-'`) —
  фикс не нужен, зафиксировано тестом как задокументированное реальное поведение.
...
- [x] **Устранить дублирование `cellText`/`normalizeText` — завершено (2/2).**
  Этап 1: `normalizeText`/`cellText` добавлены в `lib/agis-core.js` (по аналогии с
  `ruMonthNumber`, тесты в `test/lib/agis-core.test.js`) — были продублированы в
  `agis-loan-info-navbar.user.js`/`agis-protocol-income-fill.user.js` (обе версии буквально
  идентичны: nbsp — `\s+` в JS уже покрывает его, явный `.replace(/\u00a0/g, ' ')` в
  protocol-income-fill избыточен — более ранняя запись в этом файле про разницу в
  поведении была неверной, перепроверено `node -e`) и заинлайнено в
  `agis-duplicate-income.user.js`. Тег `v1.3.0` ядра вырезан и проверен (SRI-хеш совпал,
  raw.githubusercontent.com отдаёт 200).
  Этап 2: все три скрипта переведены на `window.__AGIS_CORE__.normalizeText`/`cellText`,
  локальные копии удалены; `@require` обновлён на `v1.3.0`, `@version` — patch-бамп
  (поведение не менялось). В navbar/protocol-income-fill `normalizeText` (и в
  protocol-income-fill — `cellText`) читаются через замыкание другими экспортируемыми
  тестами функциями, поэтому объявлены как `let` до guard'а и назначаются в обеих ветках
  (`require('../lib/agis-core.js')` в тесте, `window.__AGIS_CORE__` в браузере) — та же
  схема, что и `ruMonthNumber`. В duplicate-income `cellText` не читается ни одной
  экспортируемой функцией — обычная деструктуризация после проверки `window.__AGIS_CORE__`,
  без TDZ-манёвра.
...
| Волна 4.3–4.5 (CI + релиз) | средний | 1 день | ✅ завершено (кроме `release.yml` — отложен осознанно, см. выше) |
| Волна 5 (TS + тесты) | низкий, растущий | несколько вечеров | частично (unit-тесты + jsdom-тир ✅, TS не начат) |

---

## Что взять первым

1. TypeScript через `.user.ts` (см. Волна 5) — единственная существенная нетронутая задача, самая крупная и наименее приоритетная.
2. `release.yml` — только если появится конкретная причина (см. пояснение выше, почему отложен).
