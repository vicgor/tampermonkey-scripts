...
- [x] **HTML-фикстуры + jsdom (частично).** `fixtures/agis-loan-edit.html` — реальный
  ...
  Осталось: `getHeaderMap` (protocol-income-fill, читает реальный `<table><thead th>`) —
  единственное, что реально требует HTML-фикстуры; страница списка протокола ещё не
  собрана.
...
1. Собрать HTML-фикстуры для `resolveGateway`/`normalizeDate`/`cellText` (duplicate-income/protocol-income-fill) — продолжение jsdom-тира.
2. `release.yml` и проверка бампа `@version` на PR — отложены, брать по мере необходимости.
