...
| `agis-deposit-refund` | Кнопка «Вернуть депозит» на карточке займа → автозаполнение формы транзакции (сумма, OUT, тип). Отправка — вручную. На брендах кроме `moneymania` вручную не проверен | [Install](https://raw.githubusercontent.com/vicgor/tampermonkey-scripts/main/scripts/agis-deposit-refund.user.js) |
