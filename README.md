...
| `agis-deposit-refund` | Кнопка «Вернуть депозит» на карточке займа → автозаполнение формы транзакции (сумма, OUT, тип). Отправка — вручную. **Только `agis.moneymania.ru`** | [Install](https://raw.githubusercontent.com/vicgor/tampermonkey-scripts/main/scripts/agis-deposit-refund.user.js) |
