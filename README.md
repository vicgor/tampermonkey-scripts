...
**CI** (`.github/workflows/ci.yml`) гоняет `npm run lint` и `npm run format:check`
на каждый PR и на push в `main` — тот же самый `npm run lint`/`format:check`, что
локально, без отдельного конфига. Если CI зелёный, дополнительно гонять линтер
перед мержем не нужно.
